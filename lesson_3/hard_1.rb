...
def make_uuid
  ...
  uuid = ''
  segment_lengths.each_with_index do |length, idx|
    length.times { uuid << valid_chars.sample.to_s }
    uuid << '-' unless idx == segment_lengths.size - 1
  end
...
# method called is_a_number? to determine if a string is a number and asked Ben
# to use it.
...
# Not inteded to be a comprehensive implementation of the is_a_number?
# method--just need a quick and dirty way of testing if the logic of the
# above method works
def is_a_number?(string)
  string.to_i > 0  || string =='0'
end

puts dot_separated_ip_address?('0.4.5.11')
puts dot_separated_ip_address?('04511')
puts dot_separated_ip_address?('hello.world')
